...
from two_stage_model import model_simple
...
if __name__ == "__main__":
    ...
    for class_name in class_names: # create two matching lists of class directories and labels in a dictionary
        for f in (root / class_name).glob("*.tif"):
            ...
            labels.append(stage_two_idx[class_name])
    ...
    two_stage_model = model_simple(frame = FRAMES, height = HEIGHT, width = WIDTH, channels = CHANNELS)
    two_stage_model.compile(optimizer = keras.optimizers.Adam(learning_rate=1e-4), loss = keras.losses.binary_crossentropy, metrics = ["accuracy"])
    ...
    two_stage_model.save(r"\\rivendell.physics.ox.ac.uk\user\students\2024\jesu4837\summer_internship\models\stage_two.keras")
...
for x_batch, y_batch in val_ds:
    preds = two_stage_model.predict(x_batch, verbose=0)  # shape (batch, 1)
    
    # Convert sigmoid probabilities to 0/1 class predictions
...
df_cm = pd.DataFrame(
    cm,
    index=["bound", "confined"],
    columns=["bound", "confined"]
)
