...
import model_two_states
from model_two_states import model_simple
...
def load_segment(path): # load a segment of frames from a tif file and convert to numpy array
    ...
    with Image.open(path) as img:
        frames = [np.array(frame, dtype=np.float64) for frame in ImageSequence.Iterator(img)]
...
if __name__ == "__main__":
    ...
    root = Path(r"H:\summer_internship\training_pipeline_two_states\train_data")
    ...
    # small_ds = train_ds.unbatch().take(40).batch(8)
    ...
    for x, y in train_ds.unbatch().take(5):
        print(y.numpy(), x.numpy().min(), x.numpy().max())
    ...
    model.save(r"\\rivendell.physics.ox.ac.uk\user\students\2024\jesu4837\summer_internship\models\two_state.keras")
    ...
    for x_batch, y_batch in val_ds:
        preds = model.predict(x_batch, verbose=0)  # shape (batch, 1), sigmoid probs
